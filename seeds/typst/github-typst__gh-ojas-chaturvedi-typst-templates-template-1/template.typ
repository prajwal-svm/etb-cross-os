...
#let header(
  ...
  align(center,
    block[
      ...
      #link(if regex("^[a-zA-Z][-+.0-9a-zA-Z]*:") in site { site } else { "https://" + site })[#site]
    ]
  )
  v(5pt)
}
...
#let project_item(
  ...
  skills: "Programming Language 1, Skill 2",
  site: "github.com",
  date: "Jan. 1111 - Dec. 9999",
  ..points
) = {
  set block(above: 0.7em, below: 1em)
  pad(left: 1em, right: 0.5em, block[
    #link(if regex("^[a-zA-Z][-+.0-9a-zA-Z]*:") in site { site } else { "https://" + site })[*#name*] | _#skills _ #h(1fr) #date
    #list(..points)
  ])
}
...
#let skill_item(
  ...
  skills: "Git, Brawl Stars, Puzzles",
) = {
